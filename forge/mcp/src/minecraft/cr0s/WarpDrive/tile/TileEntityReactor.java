...
import cr0s.WarpDrive.jumpgate.JumpGate;
...
public class TileEntityReactor extends TileElectricBase {
    ...
    static int capacity = WarpDriveConfig.i.WC_MAX_ENERGY_VALUE * 250;
    static int maxReceive = capacity / 5;
    
    int WC_JOULES_PER_BLOCK_MODE1 = WarpDriveConfig.i.WC_ENERGY_PER_BLOCK_MODE1 * 250; 
    int WC_JOULES_PER_DISTANCE_MODE1 = WarpDriveConfig.i.WC_ENERGY_PER_DISTANCE_MODE1 * 250; 
    int WC_JOULES_PER_BLOCK_MODE2 = WarpDriveConfig.i.WC_ENERGY_PER_BLOCK_MODE2 * 250; 
    int WC_JOULES_PER_DISTANCE_MODE2 = WarpDriveConfig.i.WC_ENERGY_PER_DISTANCE_MODE2 * 250; 
    ...
    public void updateEntity()
    {
        ...
        switch (currentMode)
        {
            ...
            case MODE_GATE_JUMP:
                ...
                coreState = "Energy level: " + currentEnergyValue + " Joules";
                ...
                if (controller.isJumpFlag())
                {
                    ...
                    if (WarpDrive.instance.registry.isWarpCoreIntersectsWithOthers(this))
                    {
                        ...
                        messageToAllPlayersOnShip("The warp field intersects with another ship's field. Due to pseudo-electromagnetic interference and gravitational imbalancing, calculating a safe jump is impossible!");//technobabble
                        return;
                    }

                    if (WarpDrive.instance.cloaks.isInCloak(worldObj.provider.dimensionId, xCoord, yCoord, zCoord, false))
                    {
                    	System.out.println("[WD] Core is cloaked");
                        this.controller.setJumpFlag(false);
                        messageToAllPlayersOnShip("The warp core is currently cloaked. Due to the mechanics of cloaking via extremely dangerous teslacoils magnetostatically interrupting the warp computer's mainframe, not even to mention the highly sophisticated and specified light refraction, the ship can't jump without severe danger to the passengers and body. Disable the cloaking field to jump!"); //technobabble
                        return;                    	
                    }
    ...
    public void teleportPlayersToSpace()
    {
        if (currentMode == MODE_TELEPORT && worldObj.provider.dimensionId != WarpDrive.instance.spaceDimID)
        {
            if (worldObj.isBlockIndirectlyGettingPowered(xCoord, yCoord, zCoord))
            {
                ...
                for (Object o : list)
                {
                    if (currentEnergyValue - WC_JOULES_PER_ENTITY_TO_SPACE < 0)
                    {
                        return;
                    }
    ...
    public int calculateRequiredEnergy(int shipVolume, int jumpDistance)
    {
        ...
        {
            case MODE_BASIC_JUMP:
                energyValue = (WC_JOULES_PER_BLOCK_MODE1 * shipVolume) + (WC_JOULES_PER_DISTANCE_MODE1 * jumpDistance);
                break;

            case MODE_LONG_JUMP:
                energyValue = (WC_JOULES_PER_BLOCK_MODE2 * shipVolume) + (WC_JOULES_PER_DISTANCE_MODE2 * jumpDistance);
                break;

            case MODE_HYPERSPACE:
                energyValue = capacity / 10; // 10% of maximum
                break;

            case MODE_BEACON_JUMP:
                energyValue = capacity / 2;  // half of maximum
                break;
