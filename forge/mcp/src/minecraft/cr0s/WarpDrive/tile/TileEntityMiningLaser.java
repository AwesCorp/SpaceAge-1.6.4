...
public class TileEntityMiningLaser extends TileEntity implements IPeripheral/*, IGridMachine, ITileCable*/ { //TODO receive energy, gui
	...
	private boolean canDig(int blockID) {
		if (Block.blocksList[blockID] != null)
			return ((blockID == Block.whiteStone.blockID || blockID == WarpDriveConfig.i.iridiumID || Block.blocksList[blockID].blockResistance <= Block.obsidian.blockResistance) && blockID != WarpDriveConfig.i.MFFS_Field && blockID != Block.bedrock.blockID);
		else
	...
	public void quarry() {
		if (isMining && !isQuarry) {
			isQuarry = true;
		} else if (!isMining && !isQuarry) {
		isQuarry = true;
		delayTicksScan = 0;
		currentMode = 0;
		minerVector = new Vector3(xCoord, yCoord - 1, zCoord).add(0.5);
	...
	public Object[] callMethod(IComputerAccess computer, ILuaContext context, int method, Object[] arguments) throws Exception
	{
		switch (method)
		{
			...
			case 5: // Offset
				if (arguments.length == 1) //TODO Ask dad how to do offset
				{
