...
public class BiomeLayerVoronoiZoom extends BiomeLayer
{
  ...
  public int[] getInts(int par1, int par2, int par3, int par4)
  {
    ...
    {
      int l2 = aint[(0 + (i3 + 0) * l1)];
      int j3 = aint[(0 + (i3 + 1) * l1)];

      for (int k3 = 0; k3 < l1 - 1; k3++)
      {
        double d0 = i1 * 0.9D;
        initChunkSeed(k3 + j1 << b0, i3 + k1 << b0);
        double d1 = (nextInt(1024) / 1024.0D - 0.5D) * d0;
        double d2 = (nextInt(1024) / 1024.0D - 0.5D) * d0;
        initChunkSeed(k3 + j1 + 1 << b0, i3 + k1 << b0);
        double d3 = (nextInt(1024) / 1024.0D - 0.5D) * d0 + i1;
        double d4 = (nextInt(1024) / 1024.0D - 0.5D) * d0;
        initChunkSeed(k3 + j1 << b0, i3 + k1 + 1 << b0);
        double d5 = (nextInt(1024) / 1024.0D - 0.5D) * d0;
        double d6 = (nextInt(1024) / 1024.0D - 0.5D) * d0 + i1;
        initChunkSeed(k3 + j1 + 1 << b0, i3 + k1 + 1 << b0);
        double d7 = (nextInt(1024) / 1024.0D - 0.5D) * d0 + i1;
        double d8 = (nextInt(1024) / 1024.0D - 0.5D) * d0 + i1;
